...
## Remarks on DuckDB
This project uses DuckDB (https://duckdb.org/) as storage for the vocabulary and the generated embeddings as well as two of its extensions for the retrieval of matching subjects. The two extensions are DuckDB's Vector Similarity Search Extension (https://duckdb.org/docs/extensions/vss.html), which is used for indexing and querying the embeddings, and DuckDB's Full-Text Search Extension (https://duckdb.org/docs/extensions/full_text_search.html), which is used for indexing and querying the subject labels. Both extension allow a certain amount of configuration. The configuarion options for the FTS Extension are mainly concerning the text processing such as the type of stemmer used and an optional list of stopwords. All options can be found on the documentation page. The VSS Extension allows for some configurations regarding the HNSW index and the choice of distance metric (see documentaion for details). In this project, the 'cosine' distance and the corresponding 'array_cosine_distance' function are used. The metric and function must be explicitly specified when creating and using the index and must match in order to work. To save the created index, the configuration option for the database 'hnsw_enable_experimental_persistence=true' must be set. This is not recommended at the moment, but should not be a problem for this project as no further changes are expected once the collection has been created. Relevant and useful blog posts on the VSS Extension extension can be found here 
