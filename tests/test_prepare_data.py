...
def test_parse_vocab_reads_ttl_and_returns_dataframe(tmp_path):
    ...
    assert "http://example.org/concept/mathematics" in df["label_id"].to_list()
    assert "Mathematics" in df["label_text"].to_list()
    # All is_prefLabel should be True (since only prefLabel in test data)
    analysis_rows = df.filter(
        pl.col("label_id") == "http://example.org/concept/analysis"
    )
    assert len(analysis_rows) == 2
    assert {
        "label_id": "http://example.org/concept/analysis",
        "label_text": "Analysis",
        "is_prefLabel": True,
    } in analysis_rows.to_dicts()
    assert {
        "label_id": "http://example.org/concept/analysis",
        "label_text": "Calculus",
        "is_prefLabel": False,
